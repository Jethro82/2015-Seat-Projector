        ...
        For PartyCount = 0 To 4
            ...
            If ProportionalSwing Then
                If PartyCount <> 3 Or RegionCount = 2 Then
                    VoteAdjustment(RegionCount, PartyCount) = Val(Text1(TextBoxNo)) / (Regions(RegionCount).PartyVotes(PartyCount) / Regions(RegionCount).TotalVotes * 100)
                Else
                    VoteAdjustment(RegionCount, PartyCount) = 1
                End If
            Else
                VoteAdjustment(RegionCount, PartyCount) = Val(Text1(TextBoxNo)) / 100 - Regions(RegionCount).PartyVotes(PartyCount) / Regions(RegionCount).TotalVotes
            End If
        Next
    Next
...
Private Sub Form_Load()
    ...
    Dim TextBoxNo%
    For PartyCount = 0 To 4
        Label1(PartyCount).Caption = PartyNames(PartyCount)
        Label1(PartyCount).Top = 500 + PartyCount * 400
    Next
    
    For RegionCount = 1 To LastRegion
        Label2(RegionCount - 1).Left = 1000 + 1000 * RegionCount
        Label2(RegionCount - 1).Caption = Regions(RegionCount).RegionName
        For PartyCount = 0 To 4
            TextBoxNo = (RegionCount - 1) * 5 + PartyCount
            Text1(TextBoxNo).Top = 500 + 400 * PartyCount
            Text1(TextBoxNo).Left = 1000 + 1000 * RegionCount
            Text1(TextBoxNo).Text = Int(Regions(RegionCount).PartyVotes(PartyCount) / Regions(RegionCount).TotalVotes * 1000) / 10
            If PartyCount = 3 And RegionCount <> 2 Then Text1(TextBoxNo).Visible = False
        Next
    Next
    For TextBoxNo = TextBoxNo + 1 To 34
        Text1(TextBoxNo).Visible = False
    Next
    For RegionCount = LastRegion To 6
        Label2(RegionCount).Visible = False
    Next
End Sub
